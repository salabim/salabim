...
class Monitor:
    ...
    def as_dataframe(self, include_t: bool = True, use_datetime0=False) -> "dataframe":
        ...
        For level monitors, Monitor.as_resample_dataframe is likely more useful
        """
        try:
            import pandas as pd
        except ImportError:
            ...
            if self._video_out in ("gif", "webp"):
                if self._images:
                    ...
                    if self._video_repeat == 1:  # in case of repeat == 1, loop should not be specified (otherwise, it might show twice)
                        self._images[0].save(
                            self._video_name, disposal=2, save_all=True, append_images=self._images[1:], duration=round(1000 / self._real_fps), optimize=False
                        )
                    else:
                        self._images[0].save(
                            self._video_name,
