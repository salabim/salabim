    ...
    class X(sim.Component):
        def animation_objects(self):
            ao = sim.AnimateRectangle(
                spec=(-40, -10, 40, 10),
                fillcolor="red",
                text=lambda arg, t: f"{len(q)}.{arg.sequence_number()}",
                arg=self,
            )
            return 90, 30, ao
...
def test97():
    ...
    d2 = sim.Pdf((0, 1, 2, 3, 4, 5, 6), (18, 18, 18, 18, 18, 8, 2), "days") + sim.Cdf(
        (0, 0, 8, 10, 17, 90, 24, 100), "hours"
    )
    for i in range(0):
        print(d2())
...
def test91():
    ...
    print(
        (
            env.screen_to_usercoordinates_x(0),
            env.screen_to_usercoordinates_y(0),
            env.screen_to_usercoordinates_x(1024),
            env.screen_to_usercoordinates_y(768),
        ),
    ...
    sim.AnimateLine(
        (
            env.user_to_screencoordinates_x(10),
            env.user_to_screencoordinates_y(-10),
            env.user_to_screencoordinates_x(-10),
            env.user_to_screencoordinates_y(10),
        ),
    ...
    class PeriodMonitor(sim.Component):
        ...
        def setup(self, monitor, periods=None):
            ...
            if self.m._timestamp:
                self.perperiod = [
                    sim.MonitorTimestamp(name=self.m.name() + ".period[" + str(i) + "]", monitor=False)
                    for i in range(len(self.periods))
                ]
            else:
                self.perperiod = [
                    sim.Monitor(name=self.m.name() + ".period[" + str(i) + "]", monitor=False)
                    for i in range(len(self.periods))
                ]
...
def test82():
    ...
    y = sim.AnimateCircle(
        radius=100, x=900, y=600, linewidth=1 / env.scale(), fillcolor=("red", 100), linecolor="black", draw_arc=True
    )
    #    x = sim.Animate(circle0=(100,100, 0, 60, False), x0=900, y0=600, linewidth0=3, fillcolor0='red', linecolor0='black')
    a = sim.Animate(
        circle0=(100, 100, 0, 0, True),
        circle1=(100, None, 0, 720, True),
        x0=200,
        y0=200,
        linewidth0=3,
        fillcolor0="blue",
    ...
    class X(sim.Component):
        ...
        def animation_objects1(self, id):
            ...
            ao0 = sim.AnimateRectangle(
                spec=lambda c, t: (0, 0, 40 + 10 * t if c.index(q) <= 2 else 40, 20),
                text=self.name(),
                fillcolor=id,
                textcolor="white",
                arg=self,
...
def test58():
    ...
    for name in names:
        ...
        sim.Animate(
            text="<null string>" if name == "" else name,
            x0=x + sx / 2,
            y0=y + sy / 2,
            anchor="c",
    ...
    class X(sim.Component):
        def process(self):
            ...
            env.animation_parameters(
                animate=True, modelname="something else", background_color="90%gray", x0=-100, width=500, height=500
            )
            env.x0(-100)
            while env.now() <= 25:
...
def test8():
    ...
    class X(sim.Component):
        ...
        def process(self):
            ...
            self.rx = sim.Animate(
                x0=600, y0=300, linewidth0=1, rectangle0=(-200, -200, 200, 200), t1=10, fillcolor0="green#7f", angle1=0
            )
            ...
            self.rx = sim.Animate(
                x0=600, y0=300, linewidth0=1, rectangle0=(-200, -200, 200, 200), t1=10, fillcolor0="blue#7f", angle1=360
            )
            ...
            self.p1 = sim.AnimatePolygon(
                x0=200,
                y0=200,
                polygon0=(-100, -100, 100, -100, 100, 100, -100, 100),
                t1=25,
                x1=100,
                y1=100,
                fillcolor1="red",
            ...
            self.p2 = sim.Animate(
                x0=500, y0=500, line0=(0, 0, 100, 100), angle1=360, t1=10, linecolor0="red", linewidth0=5
            )
            self.r2 = sim.Animate(
                x0=300, y0=700, rectangle0=(-300, -300, 300, 300), fillcolor0="", linecolor0="black", linewidth0=2
            )
...
def test20():
    ...
    if Pythonista:
        for t in (
            "TimesNewRomanPSItalic-MT",
            "Times.NewRoman. PSItalic-MT",
            "TimesNewRomanPSITALIC-MT",
            "TimesNoRomanPSItalic-MT",
        ):
