...
class PrinceGenerator(sim.Component):
    def process(self):
        while True:
            yield self.hold(int(sim.Exponential(40).sample()))  # every 10-20 years othere's a new heir of the throne
            Prince()
...
class Prince(sim.Component):
    def process(self):
        ...
        if env.king is None:  # there is no king, so this prince will become king, immediately
            ...
            yield self.wait((king_died, True, -env.now()), fail_at=self.live_till)
