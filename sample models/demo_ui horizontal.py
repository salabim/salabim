...
env.start_ui(
    window_position=(10, env.height() + 40),
    window_size=(env.width(), 200),
    elements=[
        [
        ...
        [
            ...
            sg.Text(f"Pause at{env.get_time_unit('(t)')}", key="-PAUSE-AT-TEXT-"),
            sg.Input("", key="-PAUSE-AT-", size=(10, 20)),
            sg.Text(f"Pause each {env.get_time_unit('(d)')}", key="-PAUSE-EACH-TEXT-"),
        ...
        [
            sg.Text("Number of servers", key=""),
            sg.Input(env.servers.capacity(), key="-NUMBER-OF-SERVERS-", size=(4, 10)),
            sg.Text("Inter arrival time", key=""),
            sg.Input(env.mean_iat, key="-IAT-", size=(4, 10)),
